...
**Explicación**: `x` no cambia (0 a 0), solo cambia `y` (0 a 1). Por tanto, el vector apunta hacia abajo.

### Pregunta 2: Solución
**Respuesta**: c) `offset` y `stop-color`.
**Explicación**: `offset` dice "dónde" (ej 50%) y `stop-color` dice "qué color".

### Pregunta 3: Solución
**Respuesta**: b) Se mueve y estira junto con el rectángulo.
**Explicación**: Por defecto, `gradientUnits="objectBoundingBox"`. Esto significa que el sistema de coordenadas del gradiente es relativo a la caja del objeto (0 a 1). Si el objeto crece, el gradiente crece.
