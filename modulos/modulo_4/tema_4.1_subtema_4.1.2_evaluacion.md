...
**Justificación**: El `viewBox` define "qué ventana del mundo estamos mirando". Si la ventana (0-50) es más pequeña que el marco donde se muestra (0-100), la imagen se estira para llenar el marco. Es como hacer zoom.

### Pregunta 2: Solución

**Respuesta**: c) `<title>` y `<desc>`.
**Justificación**: Son los estándares W3C para accesibilidad en SVG. Funcionan como el atributo `alt` de las imágenes.

### Pregunta 3: Solución

**Respuesta**: b) Usar `<use>`.
**Justificación**: Reduce el tamaño del DOM y permite al navegador optimizar el renderizado, ya que sabe que es la misma geometría repetida.
